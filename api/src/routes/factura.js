...
router.get('/', async function (req, res, next) {
  try {
      sql="select facturas.id,to_char(facturas.fecha,'dd/mm/yyyy') as fecha,facturas.subtotal,facturas.iva,facturas.descuento,facturas.total,status.description as stsdes,"
      sql = sql + " clientes.nombre,facturas.cod_status,facturas.observ "
...
router.get('/cab', async function (req, res, next) {
  ...
  if(id) {
      try {
      sql='select facturas.id,facturas.subtotal,facturas.iva,facturas.descuento,facturas.total,facturas.desc_id,'
...
router.get('/mail', async function (req, res, next) {
  ...
  if(id) {
      try {
        ...
        sql = sql + ' join materiaprima mp on mp.id = pm.mp_id '
...
router.post('/', async function (req, res, next) {
  try {
    const { cli_id, dir_id, desc_id, fac_id, subtotal, iva, descuento,total,cod_status,observ,fecha } = req.body;
   ...
   if (!cli_id || !dir_id || !subtotal || !iva || !total || !cod_status  ) {
    ...
    if (fac_id !== 0){
      ...
      sql=`insert into facturas (cli_id,dir_id,desc_id,subtotal,iva,descuento,total,cod_status,observ,fecha) `
      sql= sql + `values (${cli_id},${dir_id},${desc_id},${subtotal},${iva},${descuento},${total},${cod_status},'${observ}','${fecha}') RETURNING id`
    }
    const records = await seq.query(sql,
      {
        logging: console.log,
