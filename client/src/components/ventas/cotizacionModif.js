...
function Formcotizacion() {
   ...
   function handleTipo(e, i) {
      ...
      if (e.target.name === "descuento") {
        cotizaciondet[i.i].descto = e.target.value;
        cotizaciondet[i.i].total = parseInt(cotizaciondet[i.i].cantidad * (cotizaciondet[i.i].precio - (cotizaciondet[i.i].precio * (e.target.value/100))));
        console.log('descto: ', cotizaciondet[i.i].descto);
        console.log('total: ', cotizaciondet[i.i].total);
        if (onChange) {
           setOnChange(false);
        } else {
           setOnChange(true);
         ...
         if (e.target.value === "0") {
            ...
            for (var z = 0; z < producto.length; z++) {
               if (parseInt(producto[z].id) === parseInt(e.target.value)) {
                  if (
                     producto[z].cod_status === 1 ||
                     producto[z].cod_status === 3
                  ) {
                     ...
                     cotizaciondet[i.i].total =
                        cotizaciondet[i.i].cantidad * cotizaciondet[i.i].precio - (cotizaciondet[i.i].precio * cotizaciondet[i.i].descto / 100 );;
                  } else {
                     Swal.fire({
                        title: "Aviso...",
